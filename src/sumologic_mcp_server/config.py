...
class SumoInstanceConfig(BaseModel):
    ...
    def validate_credentials(cls, v: str) -> str:
        ...
        if v in ['your_access_id_here', 'your_access_key_here', 'your_id', 'your_key', 'PLACEHOLDER']:
...
class ServerConfig(BaseModel):
    ...
    max_query_limit: int = Field(default=1000, ge=1, le=10000, description="Maximum results per query")
    max_search_timeout: int = Field(default=300, ge=10, le=600, description="Maximum search timeout in seconds")
    rate_limit_per_minute: int = Field(default=60, ge=1, le=1000, description="Requests per minute per tool")
