    ...
    try:
        ...
        if "error" in result_dict:
            ...
            if name == "get_account_status":
                ...
                print(
                    f"  Forecasted Continuous Ingest: {result_dict.get('forecastedContinuousIngest', 'N/A')}"
                )
                print(
                    f"  Forecasted Frequent Ingest: {result_dict.get('forecastedFrequentIngest', 'N/A')}"
                )
...
async def main():
    ...
    results.append(
        await run_tool_test("get_account_status", get_account_status(instance="default"))
    )

    await asyncio.sleep(delay)

    # Test 2: get_usage_forecast (default - contract term)
    print("\\nTest 2: get_usage_forecast (default - contract term)")
    results.append(
        await run_tool_test("get_usage_forecast", get_usage_forecast(instance="default"))
    )

    await asyncio.sleep(delay)

    # Test 3: get_usage_forecast (7 days)
    print("\\nTest 3: get_usage_forecast (7 days)")
    results.append(
        await run_tool_test(
            "get_usage_forecast", get_usage_forecast(number_of_days=7, instance="default")
        )
    )

    await asyncio.sleep(delay)

    # Test 4: get_usage_forecast (30 days)
    print("\\nTest 4: get_usage_forecast (30 days)")
    results.append(
        await run_tool_test(
            "get_usage_forecast", get_usage_forecast(number_of_days=30, instance="default")
        )
    )

    await asyncio.sleep(delay)

    # Test 5: export_usage_report (last 7 days)
    print("\\nTest 5: export_usage_report (last 7 days)")
