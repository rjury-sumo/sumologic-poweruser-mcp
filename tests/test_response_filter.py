...
class TestFilterByField:
    ...
    def test_exact_match_case_insensitive(self):
        """Test exact matching."""
        result = filter_by_field(self.items, "name", "prod-collector", exact_match=True)
        assert len(result) == 1
        assert result[0]["id"] == 1

    def test_case_sensitive(self):
        """Test case-sensitive matching."""
        result = filter_by_field(self.items, "name", "prod", case_sensitive=True)
        assert len(result) == 1
        assert result[0]["id"] == 1
...
class TestFilterByMultipleFields:
    ...
    def test_search_multiple_fields(self):
        ...
        result = filter_by_multiple_fields(self.items, "security", ["name", "description"])
        assert len(result) == 2
    ...
    def test_search_single_field(self):
        ...
        result = filter_by_multiple_fields(self.items, "security", ["name"])
        assert len(result) == 1
        assert result[0]["id"] == 1
    ...
    def test_deduplication(self):
        ...
        result = filter_by_multiple_fields(items, "test", ["name", "description"])
        assert len(result) == 1
...
class TestFilterByCustom:
    ...
    def test_complex_custom_filter(self):
        ...
        result = filter_by_custom(
            items, lambda x: x.get("enabled", False) and x.get("count", 0) > 75
        )
        assert len(result) == 1
        assert result[0]["id"] == 1
...
class TestTruncateResponse:
    ...
    def test_truncate_by_item_count(self):
        ...
        response = {"data": [{"id": i} for i in range(100)], "count": 100}
    ...
    def test_truncate_by_bytes(self):
        ...
        response = {"data": [{"id": i, "data": "x" * 1000} for i in range(100)]}
...
class TestFilterResponse:
    ...
    def test_filter_by_field(self):
        ...
        result = filter_response(self.response, field="name", value="prod")
    ...
    def test_filter_by_search_term(self):
        ...
        result = filter_response(self.response, search_term="prod", search_fields=["name"])

        assert len(result["collectors"]) == 2

    def test_custom_filter(self):
        """Test custom filter function."""
        result = filter_response(self.response, custom_filter=lambda c: c.get("alive", False))

        assert len(result["collectors"]) == 3

    def test_filter_with_max_items(self):
        """Test filtering with item limit."""
        result = filter_response(self.response, field="alive", value="true", max_items=2)
...
class TestIntegration:
    ...
    def test_realistic_collector_filtering(self):
        ...
        result = filter_response(
            response, custom_filter=lambda c: "prod" in c["name"] and c["alive"], max_items=50
        )
    ...
    def test_dashboard_search_with_truncation(self):
        ...
        result = filter_response(
            response, search_term="Security", search_fields=["name", "description"], max_items=25
        )
